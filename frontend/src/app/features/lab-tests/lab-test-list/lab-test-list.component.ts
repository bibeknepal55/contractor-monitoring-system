...
@Component({
  selector: 'app-lab-test-list', standalone: true,
  ...
  template: `
    <div class="page">
      <div class="header"><div class="hl"><h1>Lab Tests</h1><span class="count">{{total}} tests</span></div>
        <button mat-flat-button color="primary" routerLink="new" *ngIf="auth.hasPermission('LabTest.Create')"><mat-icon>add</mat-icon> Record Test</button></div>
      ...
      <div class="tc" *ngIf="!loading && items.length>0">
        <table mat-table [dataSource]="items" matSort (matSortChange)="onSort($event)">
          ...
          <ng-container matColumnDef="testCode"><th mat-header-cell *matHeaderCellDef>Code</th><td mat-cell *matCellDef="let r" style="font-weight:500;">{{r.testCode||'-'}}</td></ng-container>
          <ng-container matColumnDef="testName"><th mat-header-cell *matHeaderCellDef mat-sort-header>Test Name</th><td mat-cell *matCellDef="let r">{{r.testName||'-'}}</td></ng-container>
          ...
          <ng-container matColumnDef="testDate"><th mat-header-cell *matHeaderCellDef mat-sort-header>Test Date</th><td mat-cell *matCellDef="let r">{{r.testDate?fd(r.testDate):'-'}}</td></ng-container>
          <ng-container matColumnDef="labName"><th mat-header-cell *matHeaderCellDef>Lab</th><td mat-cell *matCellDef="let r">{{r.labName||'-'}}</td></ng-container>
          <ng-container matColumnDef="technicianName"><th mat-header-cell *matHeaderCellDef>Technician</th><td mat-cell *matCellDef="let r">{{r.technicianName||'-'}}</td></ng-container>
          <ng-container matColumnDef="actions"><th mat-header-cell *matHeaderCellDef style="text-align:center;">Actions</th><td mat-cell *matCellDef="let r" class="act"><button mat-icon-button color="primary" (click)="edit(r)" *ngIf="auth.hasPermission('LabTest.Update')"><mat-icon>edit</mat-icon></button><button mat-icon-button color="warn" (click)="del(r)" *ngIf="auth.hasPermission('LabTest.Delete')"><mat-icon>delete</mat-icon></button></td></ng-container>
      ...
      <div class="empty" *ngIf="!loading && items.length===0"><mat-icon>science</mat-icon><h3>No Lab Tests</h3><button mat-flat-button color="primary" routerLink="new">Record Test</button></div>
    </div>
  `,
...
export class LabTestListComponent implements OnInit {
  private srv=inject(LabTestService); readonly auth=inject(AuthService); private notify=inject(NotificationService); private router=inject(Router);
  items:any[]=[]; total=0; p=1; ps=10; sb='createdAt'; sd:'asc'|'desc'='desc'; s=''; loading=false; private sbj=new Subject<string>();
  c=['project','testCode','testName','category','testDate','labName','technicianName','actions'];
  ...
  applyPending(){const c=sessionStorage.getItem('labtest_updated');if(!c)return;try{const u=JSON.parse(c);const i=this.items.findIndex((x:any)=>x.id===u.id);if(i>=0)this.items[i]={...this.items[i],...u};sessionStorage.removeItem('labtest_updated')}catch(e){}}
  catColor(c:string):string{const m:any={'Concrete':'#1976d2','Soil':'#8d6e63','Steel':'#616161','Asphalt':'#424242','Water':'#0288d1','Aggregate':'#f57c00','Cement':'#78909c','Other':'#757575'};return m[c]||'#757575'}
  ...
  onPg(e:PageEvent){this.p=e.pageIndex+1;this.ps=e.pageSize;this.fetch()} edit(r:any){this.router.navigate(['/lab-tests',r.id,'edit'])}
  async del(r:any){if(!await this.notify.confirmDelete(r.testName||'Test'))return;this.srv.delete(r.id).subscribe({next:(x:any)=>{if(x.success){this.notify.success('Deleted');this.fetch()}else this.notify.error(x.message||'Failed')},error:()=>this.notify.error('Failed')})}
}